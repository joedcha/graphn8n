...
### `n8n.workflow.executions.count`/`.errors` exist because the resend above breaks naive counting

The Grafana dashboard originally derived "how many times did this workflow run" by counting the history points of `n8n.workflow.duration.last` in a time range (one point gets sent per execution, so `count()` over the range looked like a reasonable proxy). That works fine for a single selected workflow, but breaks two ways once you actually rely on it: (1) the resend behavior described above means a workflow with frequent `Wait` pauses gets the same execution re-sent every cron tick, inflating the "count" far past the real number of runs (observed inflating from ~721 to ~1440/day, i.e. converging on "once a minute" regardless of real execution frequency); (2) when a Grafana Stat panel shows multiple series at once (e.g. the `$workflow` variable set to "All"), Grafana joins them onto a shared time axis and forward-fills gaps, which corrupts `Count`/`Change Count`/`Distinct Count` field reducers identically across every series (confirmed by testing all three — this looks like a real bug in this Grafana version's reducer implementation on joined multi-series frames, not a query mistake).

`register_execution_count` in `executions.py` fixes this at the source instead of trying to work around it in Grafana: it keeps a small per-workflow `seenIds` list in `state.json` (capped at `MAX_SEEN_IDS_PER_WORKFLOW`, only needs to cover one pending episode's worth of resends) and only increments the persisted `total`/`errors` counters the first time a given execution id is seen, no matter how many times that id gets reprocessed later. The result (`n8n.workflow.executions.count[id]` / `n8n.workflow.executions.errors[id]`) is a monotonically increasing accumulator, not a per-execution event — the Grafana panel should read it with the `Last` (or `Max`) field reducer, not `Count`, and treat the number as "total since this item started being sent" (no backfill), not "total in the selected time range". `Last`/`Max` are unaffected by the join-and-forward-fill multi-series bug above, which is the other reason this design was chosen over trying to fix the count in Grafana.
